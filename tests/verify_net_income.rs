...
fn verify_net_income_amounts() {
    ...
    let json_content = fs::read_to_string(json_path).expect(
        "Failed to read JSON file. Make sure you are running 'cargo test' from the project root.",
    );

    // Deserialize into Income struct
    let income: Income =
        serde_json::from_str(&json_content).expect("Failed to deserialize JSON into Income struct");
    ...
    // Verify each field
    assert_eq!(
        income.revenue.net.value, expected_revenue,
        "Revenue mismatch"
    );
    assert_eq!(income.costs.net.value, expected_costs, "Costs mismatch");
    assert_eq!(
        income.operations.net.value, expected_operations,
        "Operations mismatch"
    );
    assert_eq!(
        income.expenses.net.value, expected_expenses,
        "Expenses mismatch"
    );
    assert_eq!(
        income.investments.net.value, expected_investments,
    ...
    assert_eq!(
        income.currency_exchange.net.value, expected_currency_exchange,
        "Currency Exchange mismatch"
    );
    assert_eq!(
        income.benefits.net.value, expected_benefits,
        "Benefits mismatch"
    );
    assert_eq!(
        income.reclassification.net.value, expected_reclassification,
        "Reclassification mismatch"
    );
    assert_eq!(
        income.income_loss_tax.net.value, expected_income_loss_tax,
